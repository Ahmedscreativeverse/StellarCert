...
impl CRLContract {
    pub fn initialize(env: Env, issuer: Address, certificate_contract: Address) {
        if env.ttl_persistent().has(&DataKey::Issuer) {
        ...
        env.ttl_persistent().set(&DataKey::Issuer, &issuer);
        env.ttl_persistent().set(&DataKey::CertContract, &certificate_contract);
        env.ttl_persistent().set(&DataKey::RevokedCertificates, &Vec::<String>::new(&env));
        env.ttl_persistent().set(&DataKey::Info, &crl_info);
    }
    ...
    pub fn revoke_certificate(
        ...
        if env.ttl_persistent().has(&revocation_key) {
        ...
        env.ttl_persistent().set(&revocation_key, &revocation_info);

        let mut revoked_certificates = Self::get_revoked_certificate_ids(&env);
        revoked_certificates.push_back(certificate_id);
        env.ttl_persistent().set(&DataKey::RevokedCertificates, &revoked_certificates);
    ...
    fn get_revoked_certificate_ids(env: &Env) -> Vec<String> {
        match env.ttl_persistent().get(&DataKey::RevokedCertificates) {
